...
Lean uses a combination of **snake_case**, **lowerCamelCase**, and **UpperCamelCase** for all its naming.

In **snake_case**, words are separated by underscores (_) and are typically all lowercase. In **lowerCamelCase**, the first word is lowercase and subsequent words start with an uppercase letter, with no separators. In **UpperCamelCase**, every word starts with an uppercase letter, also with no separators.
...
2.  **For `Prop` and `Type`:** Use **UpperCamelCase**. This is the case when a new type or a proposition is defined, such as `Nat`, `Int`, `True` or `Nonempty`. This includes inductive types, structures and classes.
3.  **For function types:** A function's naming convention matches its return value. For example, a function of type `A → B → C` is named like a term of type `C`.
4.  **For other type terms:** Almost all other type terms use **lowerCamelCase**.
5.  **Referencing `UpperCamelCase` in `snake_case`:** When an `UpperCamelCase` name is part of a `snake_case` name, use `lowerCamelCase` to reference it.
6.  **Abbreviations:** Abbreviations like `LE` are treated as a single unit, and their casing is determined by the case of the first character.
7.  **Exceptions:** Some rare exceptions exist to maintain local naming symmetry. For example, `Ne` is used instead of `NE` to follow the example of `Eq`. Other exceptions include intervals (e.g., `Set.Icc`, `Set.Iic`), where `I` is capitalized even though the convention would dictate `lowerCamelCase`.
...
theorem neZero_iff {R : Type*} [Zero R] {n : R} : NeZero n ↔ n ≠ 0 := sorry
...
This concept appears in many programming languages. The purpose of namespaces is to solve the problem of giving the same name to many different things. For example, we might want to have a theorem `min_le_right` for both integers and natural numbers. The meaning of these two theorems is different, so we want to be able to specify that they belong to different structures. This is where the concept of a namespace comes in.

A theorem name may have the form `Namespace.theorem_name` or `Namespace1.Namespace2.theorem_name`. You use the `namespace` keyword to open a namespace. All theorems defined within this namespace will be prefixed with the namespace's name. When you want to use such a theorem, you need to write it in the format `namespace_name.theorem_name`. Alternatively, you can use the `open` keyword at the beginning of your file to open certain namespaces, allowing you to use their theorems without the prefix.
...
theorem Iff.refl (a : Prop) : a ↔ a := sorry
...
theorem ZMod.wilsons_lemma (p : ℕ) [Fact (Nat.Prime p)] : ((p - 1).factorial : ZMod p) = -1 := by sorry
...
-/
variable {α : Type}
example {a : ℤ} : a + 0 = 0 := by sorry -- add_zero_eq_zero

example {a b c : ℤ} : (a + b) * c = a * c + b * c := by sorry  -- add_mul

example {a b : ℝ} : a / b = a * b⁻¹ := by sorry -- div_eq_mul_inv

example {a b c : ℝ} : a ∣ b - c → (a ∣ b ↔ a ∣ c) := by sorry  -- div_iff dvd_of_dvd_diff
...
theorem mul_add : a * (b * c) = a * b + a * c := by sorry

theorem add_sub_right_comm : a + b - c = a - b + c := by sorry
...
/-
The following content provides an introduction to some automated tactics, focusing on their functionality and illustrating them with examples. Readers can explore them on their own or use the `#help tactic` command to learn more about them.

-----

1.  One tactic that can automatically complete proofs is `simp`. In the Mathlib library, a portion of theorems are marked with the `@[simp]` attribute, for example:
-/
...
When you use `simp` in a proof, Lean automatically searches for all such theorems and tries to match the left-hand side of each theorem with your goal. If a match is found, Lean replaces the goal with the right-hand side of the equality. Note that after using `simp`, Lean will blindly try to match all theorems until it can no longer continue. As a result, the final form you get might not be what you originally wanted. A good approach is to use `simp only`, which is similar in format to `rw`. You can specify the theorems you want to use, and Lean will only use that specific set of theorems for simplification. Here are some examples:
-/
...
/-
2.  `linarith` & `nlinarith`. `linarith` is a powerful automation tactic used to handle goals involving linear order structures, such as equalities and inequalities. Its working principle is to use proof by contradiction: it first negates the conclusion and then searches for a contradiction among all the hypotheses. `linarith` is a terminal tactic, so if it successfully finds a contradiction, it ends the proof; otherwise, it throws an error. Additionally, sometimes our hypotheses are not sufficient for `linarith` to find a contradiction, so we can provide a list of arguments after it, as in `linarith [thm₁, thm₂, ...]`, to give `linarith` more assumptions to work with.

`nlinarith` is an upgraded version of `linarith` that can handle some nonlinear cases, such as expressions with squares.
...
example {x y : ℝ} (h1 : x = 5) (h2 : 1 ≤ y) : x * y + x ≥ 10 := by
  rw [h1] -- If this line is removed, `linarith` will fail, as it can only handle linear arithmetic, and multiplication is not linear. (Note the distinction between scalar multiplication and multiplication).
  linarith
...
/-
The example about multiplication shows that `linarith` cannot directly handle nonlinear situations, while the last example demonstrates `nlinarith`'s ability to handle nonlinear expressions.

-----

3.  `norm_num`. `norm_num` is used to compute numerical expressions. It's worth noting that if a variable `x` is present but we have the hypothesis `h : x = 1`, we can first use `rw [h]` to eliminate the variable from the expression before computation.
-/
