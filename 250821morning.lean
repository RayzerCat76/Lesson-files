...
-/
def center (t : Triangle) : Point := by
  exact ⟨(t.A.x+t.B.x+t.C.x)/3, (t.A.y+t.B.y+t.C.y)/3⟩

def area (t : Triangle) : ℝ := by
  have a : ℝ :=(1/2) * |t.A.x*(t.B.y - t.C.y) + t.B.x*(t.C.y - t.A.y) + t.C.x*(t.A.y - t.B.y)|
  exact a
/-
...
/-
Bonus: define the excenter (the point that have same distance to the vertices) of a triangle.
-/
def Triangle.excenter (t : Triangle) : Point := 
  sorry
