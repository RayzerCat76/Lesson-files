...
-/
/-
Recall that the elements of a set `s : Set α` are just those `x : α`, such that the proposition `s x : Prop` holds. So naturally, the membership `x ∈ s : Prop` between a term `x : α` and a set `s : Set α` should be defined as `s x : Prop`.
-/
...
-/
/-
In maths, for two sets `s` and `t`, when we say `s` is a subset of `t`, i.e., `s ⊆ t`, we mean for any element `x ∈ s`, `x ∈ t` also holds.

So is this in Lean. Given two sets `s t : Set α`, the proposition `s ⊆ t : Prop` is defined as `∀ x : α, x ∈ s → x ∈ t : Prop`. As a result, We can use `(h : s ⊆ t)` in the same way as we do w.r.t. the logical calculator like `∀`, `→`, etc.
-/
...
-/
/-
In the same way, given two sets `s t : Set α`, the intersection of them `s ∩ t : Set α` is defined as `fun x => x ∈ s ∧ x ∈ t`.
-/
...
axiom α : Type
/-
The universe set contains all the terms of a type. So by the definition of Set in Lean, for the universe set `U : Set α`, `U x` should hold for every `x`, which leads to `U` being `fun x => True`.
-/
...
/-
In Lean, we use the `Set.univ` to represent the universe set of some type `α`. Note that, `α` is an implicit parameter here.
-/
...
/-
The empty set contains no terms of a type. So by the definition of Set in Lean, for the empty set `E : Set α`, `E x` should **NOT** hold for every `x`, which leads to `E` being `fun x => False`.
-/
...
The basic format of this is `{x : t | p x}`, where `t` is a type and `p : t → Prop` is a property on type `t`. Such a set contains all the terms `x` s.t. the proposition `p x` holds.
-/
...
/-
In Lean, we use `f '' s` to represent the image of `s : Set α` under the function `f`. In particular, `f '' s` is a set of `β`, i.e., we have the expression `f '' s : Set β`.
-/
#check f '' s     -- f '' s : Set β
...
/-
In Lean, we use `f ⁻¹' t` to represent the preimage of `t : Set β` under the function `f`. In particular, `f ⁻¹' t` is a set of `α`, i.e., we have the expression `f ⁻¹' t : Set α`.
-/
#check f ⁻¹' t    -- f ⁻¹' t : Set α
...
/-
In Type Theory, for a term `x : t`, `x` is usually not a type again (not hold when `t` is `Prop`). So is this when `t` is `Set α`.

However, for a set `s : Set α`, when we write `a : s`, Lean doesn't complain!! In other words, such an expression really makes sense in Lean.
-/
variable (a : s)    -- NO ERRORS!!

/-
When we use the `#check` command to check the type of `a`, we find that the actual type of `a` is `↑s` instead of `s` itself.

This actually is a type coercion. `↑s` is a type, while `s` is not. When we write `a : s` down, Lean knows that we need a type in the position of `s`, but `s` is not a type. So Lean automatically tries to coerce `s` into a type, and there `↑s` is.
-/
#check a            -- a : ↑s
...
Given a type `α`, we can take some terms of `α` by a property `p : α → Prop` to form a new type. Such a new type is called a subtype of `α`.

For example, if we set a property `p : ℕ → Prop` to be `fun n => n ≤ 9`, then the related subtype of `ℕ` contains the natural numbers no more than `9`.

We can use `{x : α // p x}` to represent such a subtype. A term `a : {x : α // p x}` can be viewed as a pair, with two fields `.val` and `.property`.
...
-/
/-
This is a nested subtype example, which is commonly used in maths formalization. Basically speaking, it is just a subtype of a subtype. There's nothing special here. Only need to keep it clear that whose `.val` and `.property` we are using.
-/
#check ℝ                    -- ℝ : Type
...
section coercion
/-
As it is showed in the subtype section. When the type of the parameters offered and the type wanted do not match, Lean attempts to do a coercion, trying to find a correponding term with the type wanted.

If it succeeds, it marks a arrow in front of the corresponding term found, like the `↑s` above and the `↑1 : ℝ` below. If it fails, Lean just throws an error, reporting such a mismatching.
-/
def f : ℝ → ℝ := fun x => x + 1
...
/-
`ext` is short for "extensionality". When we want to prove two sets `s t : Set α` are equal, we can use `ext x` to change the goal from `s = t` to `x ∈ s ↔ x ∈ t`, where `x` is the name of the term introduced and can be customized as we want.
-/
